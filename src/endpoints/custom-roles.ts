...
export class CustomRoles {
     ...
     * @param roleId The custom role ID to update
     * @param body The organization ID the custom role belongs to, and the fields to update
     * @returns Promise with the updated custom role
     */
    async update(roleId: number, body: UpdateCustomRoleBody): Promise<CustomRole> {
        return (
            await this.#fetch<CustomRoleResponse>(`/users/custom-roles/${roleId}`, {
                method: 'PATCH',
                body,
            })
        ).role;
     ...
     * @param roleId The custom role ID to delete
     * @param body The organization ID the custom role belongs to
     * @returns Promise with the ID of the deleted custom role
     */
    async delete(roleId: number, body: DeleteCustomRoleBody): Promise<number> {
        return (
            await this.#fetch<DeleteCustomRoleResponse>(`/users/custom-roles/${roleId}`, {
                method: 'DELETE',
                body,
            })
        ).roleId;
